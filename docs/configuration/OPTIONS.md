...
| `color`           | string   | Optional. Robot artwork color: `white` (default) or `black`. On the LR3 these are its Beige and Grey colorways.                                                                                                                                                |
...
| `features`        | string[] | Optional. Feature flags. `percentage` — show fill % on litter and waste gauges. `illustrated` — robot illustration whose litter/waste zones fill in proportion to their level, instead of the marketing image and bars. See [Feature flags](FEATURE-FLAGS.md). |
