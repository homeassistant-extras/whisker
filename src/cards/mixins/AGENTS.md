...
- **`subscribe-entity-state-mixin.ts`** — `SubscribeEntityStateMixin`. Subscribes to entity state changes via `subscribe_entities` through the entity subscription manager. Only notifies on meaningful changes (state/attributes), not context/last_updated. Set `entity` to specify which entity to watch; read `state` for the current subscribed entity state (undefined when not subscribed). Sets up the subscription in `connectedCallback` and tears it down in `disconnectedCallback`. Also exports `SubscribeEntityStateElement` and the `Constructor<T>` helper used by the other mixin. Used by gauge, status, and footer-item (composed with `HassConfigMixin`).
