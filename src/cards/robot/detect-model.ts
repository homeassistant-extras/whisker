...
export type ModelKey = 'lr3' | 'lr4' | 'lr5' | 'lr5-pro' | 'lre';
 ...
 * encodes the hardware (LR3 → lr3, LR4 → lr4, LR5 → lr5, LRE → Litter-Robot
 * Evo) — while the model string only distinguishes the LR5 "Pro" variant. Falls
 * back to parsing the model string when no (recognized) serial is available.
 */
export const detectModelKey = (
  ...
  if (s.startsWith('LR3')) return 'lr3';
